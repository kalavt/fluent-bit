...
int s3_queue_recover_from_database(struct flb_s3 *ctx)
{
    ...
    while (1) {
        ...
        /* Generate s3_key for this file to complete the upload */
        flb_sds_t s3_key = flb_get_s3_key(ctx->s3_key_format, file_created, tag,
                                          ctx->tag_delimiters, ctx->seq_index, file_path);
        if (!s3_key) {
            flb_plg_warn(ctx->ins, "Recovery: failed to generate s3_key for ready file_id=%" PRIu64,
                        file_id);
            goto ready_cleanup;
        }
